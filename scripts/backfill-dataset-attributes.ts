...
const VARIABLE_STOPLIST = new Set([
  ...
  'date start', 'date end', 'start date', 'end date', 'location id', 'location name', 'sample name', 'depth id',
])
...
function normalizeVariable(name: string): string | null {
  ...
  // dd.csv meta-vocabulary: the dictionary describing its own format
  if (/^(unit|units|definition|standard|data type|column or row name|file description|missing value code|missing value codes|data orientation|contact|header rows|term type|column or row long name|file name|file format|field name)$/.test(v)) return null
  return v
}
