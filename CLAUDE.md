...
scripts/
  ...
  backfill-dataset-attributes.ts — Datasets browse Tier-2 backfill: formats (SDP→geotiff + download_url extension + EML physical) into datasets_data_format; keywords (EML <keyword> harvest via DataONE object API; attributeLists are rare on ESS-DIVE) into datasets.keywords text[]. (Variables moved to extract-dataset-variables-llm.ts.) Idempotent; --dry-run/--limit/--force/--target=neon.
  extract-dataset-variables-llm.ts — LLM (opus-5) extraction from dataset metadata, one pass per dataset: measured variables (datasets.variables, the /datasets facet) + evidence-gated units (variable_units — only when the metadata states them, verbatim quote required) + GCMD Science Keywords paths (gcmd_variables; validated verbatim against the pruned taxonomy in scripts/data/gcmd-science-keywords.json) + data-collection years (fills temporal_extent_start/end ONLY where NULL; never pub dates) + data_ongoing + temporal_resolution + cited_references jsonb (companion papers w/ DOI + evidence). Incremental: gcmd_variables IS NULL = unprocessed, so new datasets are picked up automatically (runs in pipeline ENTITIES phase). Prompt-cached taxonomy block; ~$0.02/dataset. --dry-run/--limit/--force/--report/--model; --sync-neon copies local results to Neon by DOI/title (don't re-extract there).
  link-dataset-citations.ts   — Match cited_references to publications (DOI exact, then title word_similarity > 0.5) → datasets_rels path='relatedPublications' (the haspub chip + Related Works). Idempotent; runs after extraction in pipeline ENTITIES phase. --dry-run/--target=neon.
  score-dataset-origin.ts     — Dataset provenance flag (rmbl_origin tri-state 'yes'/'no'/NULL like rmbl_research): SDP/ESS-DIVE priors + basin place markers + creator overlap with RMBL-research authors − global-product markers → rmbl_origin_score; --apply assigns flags (fills NULLs only, curation-aware; --force reassigns). Runs in pipeline ENTITIES phase. Admin sidebar select + read-only score on Datasets.
  assess-companion-reuse.ts   — FAIR R-rate Phase 2 (companion-paper indirection): for RMBL-origin datasets with DOI'd companion pubs, fetch S2 citing works WITH citation contexts, LLM-classify data_used/mention/unclear (mention dropped), independence vs author registry → dataset_reuse_events channel='companion_forward' (scan markers confidence=0 make re-runs incremental). ~$0.002/citing paper.
  # Re-use tracking runs in pipeline CITATIONS phase: Phase 1 --stale-days=30, Phase 2 --stale-days=90; push to Neon with sync-bulk --only=reuse. Official R-rate denominator: rmbl_origin='yes' AND doi IS NOT NULL (see specification/dataset-reuse-design.md).
  assess-dataset-reuse.ts     — FAIR R-rate Phase 1 (see specification/dataset-reuse-design.md): builds dataset_reuse_events from internal datasets_rels links + OpenAlex citing works (cited DOI'd datasets), classifies independence vs the author registry (same_group/collaborators/independent via co-authorship distance), rolls up reuse_internal_count/reuse_external_count/reuse_independent on datasets. Deterministic, no LLM; Phase 2 (companion-paper forward citations + LLM use-classification) not yet built. --dry-run/--limit/--skip-openalex.
  ...
  tag-student-authors.ts      — Student-author auto-detection → publication_student_authors (custom SQL table keyed on author_name so it survives author rebuilds; student papers + theses structural seed; REU tags come from tag-reu-authors.ts and are not propagated by its windowed inference; --dry-run/--target=neon). Feeds /metrics dashboard.
  tag-reu-authors.ts          — REU tags (student_program='reu') on peer-reviewed pubs from two PII inputs in gitignored scripts/data/private/: the REU program's own pub list (bold = REU author; matched by DOI, else title containment, loosened when first author agrees) + the 1991–2020 cohort roster (surname + initial + mentor co-author, within 4 yrs of cohort). Idempotent re-derive; curated rows untouched; review list → scripts/output/reu-pub-list-unmatched.txt. --dry-run/--target=neon.
  extract-reu-cohort.ts       — Derive 2021+ REU cohorts (the official roster ends 2020) from RMBL student papers: Claude reads the cover block + acknowledgments; a student counts on a verified REU quote OR an REU program line ("Full-Time Independent Research/REU"), minus anyone already a mentor/PI by that year unless explicitly verified. Calibrated vs the roster 2015–2019: ~70% recall, 96% precision; cover pages stopped carrying program lines after ~2023, so 2024+ recall is lower. Output → gitignored scripts/data/private/reu-roster-derived.csv (read by tag-reu-authors.ts); per-paper cache in scripts/output/. --calibrate re-scores; ~$2.40 for 2015–2025.
  classify-funding-programs.ts — SFA / SAIL tri-state flags (publications.sfa_program / sail_program) for RMBL-research articles ≥2012. SFA = on the Watershed Function SFA's own publication list (authoritative, Ian 2026-09-24; snapshot scripts/data/sfa-publications-YYYY-MM.json, --refresh-sfa-list re-fetches watershed.lbl.gov); listed → yes, else no. The acknowledgment classifier still runs for SFA but only as evidence (evidence.sfa.ack) → worklist of acknowledged-but-unlisted papers in scripts/output/sfa-list-worklist.csv. SAIL = classifier: Claude reads ±700 chars around cues, 'yes' needs a quote verified against the text (tolerant of two-column PDF interleaving); no cue + acknowledgments present → 'no'; no usable text → project links within the program era. Curation-aware; incremental on funding_programs_checked_at; --sfa-list-only re-applies the list with no LLM. ~$1.60 full corpus. --sync-neon copies local results by DOI/title (don't re-classify there).
  ...
  ingest-student-papers.ts    — Ingest one cohort's student papers from the education program's S3 drop (--year=YYYY --source="s3://…/"; one folder per student). Picks each student's final paper (duplicate folders collapsed, PDF over Word), converts Word → PDF with LibreOffice (`brew install --cask libreoffice`), pdftotext in reading order, Claude extracts title/students/mentors/abstract/keywords (title + abstract verified against the text). Senior co-authors (research-plan PIs, known mentors) become mentors; anonymous covers take the student from the folder name; REU students with no mentor take them from the REU abstracts doc, which also becomes the private official roster scripts/data/private/reu-roster-YYYY.csv. Inserts student_paper / rmbl_research=yes / rmbl_owned, publishes PDFs to rmbl-hub-pdfs/publications/student-papers/YYYY/<Last>_<First>.pdf (named by student, not id — ids differ local vs Neon). Idempotent; --dry-run; --target=neon. Then tag-student-authors.ts + generate-embeddings.ts (Neon: DATABASE_URL=$NEON_DIRECT_URL). 2026 cohort: 40 papers.
...
- **Rate limiting**: Per-IP sliding window on /api/v1/* (60/min general, 10/min expensive, 15/hr flags)
...
- **True duplicates: merge, don't delete** — `scripts/merge-duplicate-publications.ts --pairs=keep:drop,…` moves everything attached to the dropped copy (citations both ways, entity mentions, project/dataset/author links, flags, student tags, frontier cites, chunks, …) onto the keeper, unions curated_fields, fills NULL columns, writes the same tombstone the admin delete does, then deletes — one transaction per DB, run local then `--target=neon`, `--dry-run` rolls back. A plain admin delete cascades those rows away. Used 2026-09-25 for the 6 legacy-DB duplicate pairs.
