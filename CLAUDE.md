...
scripts/
  ...
  backfill-dataset-attributes.ts — Datasets browse Tier-2 backfill: formats (SDP→geotiff + download_url extension + EML physical) into datasets_data_format; keywords (EML <keyword> harvest via DataONE object API; attributeLists are rare on ESS-DIVE) into datasets.keywords text[]; variables (real measured variables from ESS-DIVE dd.csv data dictionaries — resolve DOI as seriesId → newest metadata's documents list → *_dd.csv files from the member node → Column_or_Row_Name) into datasets.variables text[]. Idempotent; --dry-run/--limit/--force/--target=neon.
