...
scripts/
  ...
  classify-funding-programs.ts — SFA / SAIL tri-state flags (publications.sfa_program / sail_program) for RMBL-research articles ≥2012: Claude reads ±700 chars around funding cues; a 'yes' needs a quote verified against the text (tolerant of two-column PDF interleaving); no cue + acknowledgments present → 'no'; no usable text → project links, only within each program's era (SFA ≥2017, SAIL ≥2021). Curation-aware; incremental on funding_programs_checked_at. ~$1.60 full corpus. --sync-neon copies local results by DOI/title (don't re-classify there).
