...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!s) return { title: 'Story — RMBL Knowledge Commons' }
  const desc = s.summary ? String(s.summary).slice(0, 200) : `${STORY_TYPE_LABELS[s.story_type] || 'Story'} from the RMBL Knowledge Commons`
  return {
    title: `${s.title} — RMBL Knowledge Commons`,
    description: desc,
    openGraph: { title: s.title, description: desc, url: `https://rmblknowledgecommons.org/stories/${id}` },
  }
}
