...
export async function GET() {
  ...
  `)
  const header = 'year,total,rmbl_research,with_student_authors,reu_students'
  const body = rows.map((r: any) => `${r.year},${r.total},${r.rmbl_research},${r.with_student_authors},${r.reu_students}`).join('\n')
