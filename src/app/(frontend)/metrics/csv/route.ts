...
export async function GET() {
  const { rows } = await getDb().query(`
    SELECT p.year::int AS year,
           ...
           count(*) FILTER (WHERE sa.publication_id IS NOT NULL)::int AS with_student_authors,
  ...
  `)
  const header = 'year,total,rmbl_research,peer_reviewed_rmbl,student_papers_theses,with_student_authors,reu_students'
  const body = rows.map((r: any) => `${r.year},${r.total},${r.rmbl_research},${r.peer_reviewed_rmbl},${r.student_papers_theses},${r.with_student_authors},${r.reu_students}`).join('\n')
