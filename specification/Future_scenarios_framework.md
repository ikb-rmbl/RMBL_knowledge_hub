...
- **Research model.** The vast majority of basin research is driven by a community of guest scientists, most based at academic institutions elsewhere. RMBL itself does not generally run principal-investigator-led research programs in the conventional sense. **Scenarios should not assume RMBL hires its own research scientists at scale.** When a scenario describes "an endowed program in X" it typically means infrastructure, capacity, and stewardship that enables guest-scientist work on X — not RMBL-employed researchers running their own research portfolio.
- **Staff.** RMBL operates with a relatively small staff. The technical core (approximately 4 people as of 2026) includes a geospatial data scientist, a GIS manager, and instrumentation-focused technicians. This is the staff that maintains the catalytic infrastructure on which guest-scientist research depends. Scenarios that imply significant staff expansion should be checked against realistic nonprofit-organizational growth expectations.
- **Investment philosophy.** Historically, RMBL focuses on catalytic investments that serve a broad scope of research — physical and data infrastructure, instrumentation, archival systems, facilities — rather than promoting particular research projects. RMBL is responsive to guest scientists' needs rather than directive about what they should study. Campaign deliverables that fit this philosophy are described accurately as "infrastructure," "capacity," "stewardship," or "facility" rather than as "research programs RMBL runs."
- **Community priority.** RMBL is the oldest and one of the largest nonprofit organizations in its part of Colorado. Serving the community directly is an institutional priority that exists alongside (and sometimes shapes) its scientific mission. Scenarios should treat community engagement as a real strategic dimension, not an optional add-on.
...
### 3.2 Time-bound: the current moment (as of v0.3, 2026)
...
- "Endowed program" means catalytic infrastructure, not RMBL-employed PIs
- "Translation function" or "research staff" are described in scales realistic to a small-staff nonprofit
...
Scenarios drafted without §3 grounding tend toward generic field-station templates — large research-program staffing, hypothetical federal-contraction scenarios, light community treatment, abstract independence language. Scenarios drafted with §3 grounding read as actually-RMBL.
...
Every frontier below should be read through the §3.1 operating-model lens: RMBL supports each by investing in **catalytic infrastructure** — physical facilities, data systems, instrumentation, archival capacity, partnership coordination, technical-staff capacity — that enables the guest-scientist community to advance the frontier. RMBL does not generally itself run the research that advances the frontier. Where an entry below describes "an endowed program," read this as "the infrastructure and capacity that allow a community of guest scientists to do the work" rather than as "RMBL employees doing the research." This translation matters for scenario authoring and is enforced in the `frontier_support_strategies` field.
...
- **F.innov.3 — AI-assisted retrospective synthesis of long basin records.** Using AI tools to extract findings from a century of accumulated basin records that prior generations could not analyze at the scale now possible. Catalytic support: archival digitization, geospatial-data-scientist capacity (a strength of RMBL's existing technical staff), tool development, methodological partnerships with guest scientists doing the analysis. Depends on F.cont.1 for source material.
